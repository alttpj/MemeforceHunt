...
public class ULIDDeserializerTest extends AbstractJacksonTest {

  @Test
  public void testDeserialze() throws IOException {
    // given
    final ULIDDeserializer deserializer = new ULIDDeserializer();
    final ULID.Value original = new ULID().nextValue();
    when(getParser().getText()).then(args -> original.toString());

    // when
    final ULID.Value deserialize = deserializer.deserialize(getParser(), getContext());
